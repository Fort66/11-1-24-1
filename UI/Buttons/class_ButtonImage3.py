...
class ButtonImage3:
    ...
    def __post_init__(
        self, font: str, image: str, shadingImage: str, disabledImage: str
    ):
        self.image = flip(
            scale(load(image).convert_alpha(), self.size),
            self.flipHorizontal,
            self.flipVertical,
        )
        self.rect = self.image.get_rect(x=self.pos[0], y=self.pos[1])

        self.shadingImage = flip(
            scale(load(shadingImage).convert_alpha(), self.size),
            self.flipHorizontal,
            self.flipVertical,
        )

        self.disabledImage = flip(
            scale(load(disabledImage).convert_alpha(), self.size),
            self.flipHorizontal,
            self.flipVertical,
        )

        self.font = (
            pg.font.Font(font, self.font_size)
            if font
            else pg.font.SysFont("Arial", self.font_size)
        )
            ...
            if event.type == MOUSEBUTTONDOWN:
                if event.button == 1 and self.isHovered:
                    ...
                    if self.onClickReference:
                        self.onClickReference(
                            *self.referenceArgs, **self.referenceKwargs
                        )
    ...
    def update(self, surface):
        ...
        if self.text:
            if self.onEnabled:
                ...
                textSurface.fill(
                    self.disabledColorText, special_flags=pg.BLEND_RGBA_MULT
                )
